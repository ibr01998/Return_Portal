...
//                $response = $this->ReturnModel->setApi($data, 'createLabel');
                ...
                if ($data[0]['shipping_address']['zip'] !=  $_SESSION['postcode']){
                    echo view('navbar');
                ...
                if (empty($data[0]['shipped_at']['date'])){
                    ...
                    if($_SESSION['settings']['return_limit_day'] == true && strtotime($data[0]['shipped_at']['date']) < strtotime('-'.$_SESSION['settings']['return_day_amount'].' days')){
                        echo view('navbar');
    ...
    public function Product()
    {
        ...
        {
            if(isset($_SESSION['lang']) && isset($_SESSION['settings']["return_reasons_".$_SESSION['lang'].""])){
                $result = substr($_SESSION['settings']["return_reasons_".$_SESSION['lang'].""], 1, -1);
                $string = str_replace('"', '', $result);
                $_SESSION['reasons'] = explode(",", $string);
            }else{
                $result = substr($_SESSION['settings']["return_reasons_".$this->request->getLocale()], 1, -1);
                $string = str_replace('"', '', $result);
                $_SESSION['reasons'] = explode(",", $string);
            }

            echo view('navbar', ['logo' => $_SESSION['settings']['return_settings_logo'], 'background' => $_SESSION['settings']['return_settings_background']]);
            echo view('Retourform/product.php',['items' => $_SESSION['items'], 'color' => $_SESSION['settings']['return_settings_color'], 'reasons' => $_SESSION['reasons']]);
        }
        ...
        if(empty($tmp) || !isset($_SESSION['shop_id'])){
            ...
            if(empty($items)){
                ...
                if($response){
                    ...
                    echo view('navbar', ['logo' => $_SESSION['settings']['return_settings_logo'], 'background' => $_SESSION['settings']['return_settings_background']]);
                    echo view('Retourform/done.php', ['color' => $_SESSION['settings']['return_settings_color'], 'text' => $_SESSION['settings']['return_confirmation_text']]);
                }else{
                    echo view('navbar', ['logo' => $_SESSION['settings']['return_settings_logo'], 'background' => $_SESSION['settings']['return_settings_background']]);
